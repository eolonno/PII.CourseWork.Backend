        ...
        public override void Intercept(IInvocation invocation) //invocation => method
        {
            ...
            var arguments =
                invocation.Arguments
                    .ToList(); //arguments = methodun parametreleri. Bunu, parametrelerin eleman olduğu bir liste şeklinde veriyoruz.
            var key =
                $"{methodName}({string.Join(",", arguments.Select(x => x?.ToString() ?? "<Null>"))})"; // keyi üretirken eğer parametre varsa parametreleri de ekle. yoksa eğer null olarak bırak parametre yerlerini. 
            if (_cacheManager.IsAdd(key)) //cache mevcut mu?
            {
                invocation.ReturnValue =
                    _cacheManager
                        .Get(key); //cache varmış, o zaman key değeri ile verilen cacheyi memoryden get edip methodun hazırda bulunan return valuesi olarak gönder gitsin.
                return;
            }
